...
export default function SongGenerator() {
  ...
  // refresh will not re-trigger generation.
  const didAutoGenerate = useRef(false);
  useEffect(() => {
    if (didAutoGenerate.current) return;
    if (searchParams.get("generate") !== "true") return;
    didAutoGenerate.current = true;
    // Consume the param before firing so refresh is safe.
    const next = new URLSearchParams(searchParams);
    next.delete("generate");
    navigate(
      { pathname: `/music/lyrics/${messageId}/songs`, search: next.toString() },
      { replace: true }
    );
    handleGenerate();
